    ...
    public static class IngressPoint
    {
        ...
        public static class PatchExplosion
        {
            ...
            public static void Postfix(ref Explosion __instance)
            {
                ...
                if (PatchExplosion.s_VisibleHits != null && PatchExplosion.s_VisibleHits.Count > 0)
                {
                    ...
                    // ExplosionNerf.IngressPoint.DebugPrint("list");
                    /* var ordered = myList.OrderBy(o => (float)o.GetType().GetField("sqDist", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).GetValue(o));
                    // ExplosionNerf.IngressPoint.DebugPrint("sort?");
                    List<object> SortedList = ordered.ToList();
                    // ExplosionNerf.IngressPoint.DebugPrint("back to list");
                    PatchExplosion.s_VisibleHits.Clear();
                    // ExplosionNerf.IngressPoint.DebugPrint("clearing");
                    for (int i = 0; i < SortedList.Count; i++)
                    {
                        PatchExplosion.s_VisibleHits[i] = SortedList[i];
                    }
                    // ExplosionNerf.IngressPoint.DebugPrint("remapping");
                    PatchExplosion.hitDict.SetValue(null, PatchExplosion.s_VisibleHits); */
                }
            }
        }
        ...
        public static class PatchDamage
        {
            ...
            private static ManDamage.DamageInfo NewDamageInfo(Damageable directHit, Damageable damageTarget, TankBlock targetBlock)
            {
                ...
                // Console.WriteLine("7");
                damage *= (float)targetBlock.GetComponent<ModuleDamage>().m_DamageDetachFragility;
